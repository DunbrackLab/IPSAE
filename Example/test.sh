...
mkdir -p "${af2_dir}"
af2_sample_id='5b8c'
af2_suffix='alphafold2_multimer_v3_model_4_seed_000'

# gunzip -k "${self_dir}/${af2_sample_id}_scores_${af2_suffix}.json.gz" -c >"${af2_dir}/${af2_sample_id}_scores_${af2_suffix}.json"
cp "${self_dir}/${af2_sample_id}_scores_${af2_suffix}.json" "${af2_dir}/"
...
cd "${af2_dir}"
fd --exact-depth 1 -t f '.*_15_15.*' -x sh -c 'if ! cmp -s {} ../{}; then code --diff {} ../{}; fi'
...
cp "${self_dir}/fold_${af3_sample_id}_summary_confidences_0.json" "${af3_dir}/"
...
cd "${af3_dir}"
fd --exact-depth 1 -t f '.*_10_10.*' -x sh -c 'if ! cmp -s {} ../{}; then code --diff {} ../{}; fi'
